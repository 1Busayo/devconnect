...
router.get('/me',auth, async(req,res)=> {
   ...
   if(!profile){
       return res.status(400).json({msg:'There is no profile for this useer'});
   }
...
const options= {
    uri:`https://api.github.com/users/${req.params.username}/repos?per_page=5&sort=created:asc&client_id=${config.get('githubClientId')}&client_secrets=${config.get('githubSecret')}`,
    method:'GET',
